...
let isScreenshred = false;
const screenShareBtn = document.getElementById("screenShareBtn");
screenShareBtn.addEventListener("click", screehsahre);
endCallBtn.disabled = true;
...
function connectToEngageDigital() {
  ...
  if (isEngageDigitalSdkLoaded) {
    ...
    engageDigitalClient = new window.EngageDigital.EngageDigitalClient(
      userIdentity,
      engageDomain,
      config
    );
...
function registerForEngageDigitalClientEvents() {
  ...
  engageDigitalClient.addEventHandler("connected", () => {
    ...
    sipIdentityDiv.innerText =
      "Your Sip Identity : " + engageDigitalClient.getUri().toString();
  });
...
function onNewEngageSession(session) {
  console.log(
    "Got newRTCSession event direction is %s",
  ...
  engageDigitalSession.addEventHandler("connected", () => {
    ...
    if (isCollocation) {
      engageDigitalSession.toggleVideo();
    }
  });
  ...
  engageDigitalSession.addEventHandler("collocation", () => {
    isCollocation = true;
    screenShareBtn.disabled = true;
    updateStatus("Call: Remote party disconnected due to collocation");
    console.log("Call: collcoation occured....");
    setCallControlButtonsDisableStatus({ make: true, end: true });
  });
  ...
  engageDigitalSession.addEventHandler("screensharestarted", () => {
    console.log("Got screensharestarted event");
    screenShareBtn.innerText = "Stop Sharing";
  });

  engageDigitalSession.addEventHandler("screensharestopped", ({ stream }) => {
    console.log("Got screensharestopped event");
    screenShareBtn.innerText = "Screen Share";
  });
...
function handleIncomingCall() {
  ...
  if (!isCollocation) {
    showIncomingCallDialog(incomingCallAcceptAction, incomingCallRejectAction);
      ...
      let $confirm = $("#incomingCallDialogCollocation");
      $confirm.modal("show");
      $("#collocationText").html(
        "Re-connecting..." +
          engageDigitalSession.getRemoteId().uri.toString()
      );
      setTimeout(()=>{$confirm.modal("hide");}, 5000);
  }
}
...
function loadEngageDigitalSDK(engageDomain) {
  ...
  sdkScriptElement.addEventListener("error", () => {
    updateStatus(
      `Failed to load ${sdkScriptElement.src}. Is the given domain proper?`
    );
  });
}
